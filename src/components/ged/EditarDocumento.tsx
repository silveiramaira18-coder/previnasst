...
export function EditarDocumento({ doc, tabela, tipos, onSalvo }: { doc: Documento; tabela: Tabela; tipos: readonly string[]; onSalvo: () => void }) {
  ...
  const salvar = useMutation({
    ...
    onSuccess: () => { toast.success("Dados do documento atualizados"); setAberto(false); onSalvo(); },
    ...
    <Dialog open={aberto} onOpenChange={setAberto}>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4">
          <div className="space-y-1.5"><Label>Título</Label><Input className="h-12" maxLength={150} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5"><Label>Data de emissão</Label><Input type="date" className="h-12" value={form.issue_date} onChange={(e) => setForm({ ...form, issue_date: e.target.value })} /></div>
            <div className="space-y-1.5"><Label>Data de validade</Label><Input type="date" className="h-12" value={form.expiration_date} onChange={(e) => setForm({ ...form, expiration_date: e.target.value })} /></div>
          </div>
        </div>
